...
uses
  ...
  Messages, Graphics, LCLIntf, CPort, CPortEsc,windows,LResources;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (Value.Connected) and (not (csDesigning in ComponentState))
...
begin
  ...
  Rectangle.Top:=Row;
  Rectangle.Right:=FOwner.Columns;
  Rectangle.Bottom:=Row;
  if FOwner.DoubleBuffered then
    FOwner.Invalidate
  else
    FOwner.InvalidatePortion(Rectangle);
end;
...
procedure TCustomComTerminal.Write(const Buffer; Size: Integer);
var
  I: Integer;
  Res: TEscapeResult;
  Ch: Char;
begin
  ...
  try
    ...
    begin
      Ch := Char(Pointer(Integer(@Buffer) + I)^);
