 ...
 * for Free Pascal 2.0.4                              *
 * Writen by HILD Gérald, 2007                        *
 * This work based by the Original comport            *
...
uses
  ...
  {$IFDEF WIN32}
  Windows, Messages, IniFiles, Registry;
  {$ENDIF}
  {$IFDEF LINUX}
  libc, KernelIoCtl;
  {$ENDIF}
...
uses
  //CPortSetup,  {WinSpool,} gnugettext;
  CPortSetup;
...
begin
  ...
  EventHandles[1] := Overlap.hEvent;
  repeat
    // wait for event to occur on serial port
    WaitCommEvent(FComPort.Handle, Mask, @Overlap);
    Signaled := WaitForMultipleObjects(2, @EventHandles, False, INFINITE);
    // if event occurs, dispatch it
    if (Signaled = WAIT_OBJECT_0 + 1)
      and GetOverlappedResult(FComPort.Handle, Overlap, BytesTrans, False)
    then
    begin
...
begin
  ...
  if FConnected and (FUpdateCount = 0) and
    not ((csDesigning in ComponentState) or (csLoading in ComponentState)) then
  begin
    Timeouts1.ReadIntervalTimeout := GetTOValue(FTimeouts.ReadInterval);
    Timeouts1.ReadTotalTimeoutMultiplier := GetTOValue(FTimeouts.ReadTotalMultiplier);
    Timeouts1.ReadTotalTimeoutConstant := GetTOValue(FTimeouts.ReadTotalConstant);
    Timeouts1.WriteTotalTimeoutMultiplier := GetTOValue(FTimeouts.WriteTotalMultiplier);
    Timeouts1.WriteTotalTimeoutConstant := GetTOValue(FTimeouts.WriteTotalConstant);

    // apply settings
    if not SetCommTimeouts(FHandle, Timeouts1) then
...
begin
  ...
  Success := WriteFile(FHandle, Buffer, Count, BytesTrans, @AsyncPtr^.Overlap)
...
begin
  ...
  Success := ReadFile(FHandle, Buffer, Count, BytesTrans, @AsyncPtr^.Overlap)
    or (GetLastError = ERROR_IO_PENDING);
...
begin
  ...
  Signaled := WaitForSingleObject(AsyncPtr^.Overlap.hEvent, INFINITE);
  Success := (Signaled = WAIT_OBJECT_0) and
      (GetOverlappedResult(FHandle, AsyncPtr^.Overlap, BytesTrans, False));
...
begin
  ...
  Result := GetOverlappedResult(FHandle, AsyncPtr^.Overlap, BytesTrans, False);
  if not Result then
...
begin
  ...
  try
    ...
    Success := WaitCommEvent(FHandle, Mask, @Overlap);
